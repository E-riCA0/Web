...
public class StudentActionNo extends HttpServlet{
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if ((int)session.getAttribute("isChosen")==1){
			if ((int)session.getAttribute("isNo")==1) {
				
			}
			else {
				ccd.AddNo(ccd.findByName((String)session.getAttribute("CourseName")));
				ccd.DeleteYes(ccd.findByName((String)session.getAttribute("CourseName")));
				session.setAttribute("isNo",1);
		...
		req.setAttribute("msgNo","I Don't Get It !");
